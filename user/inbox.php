		...
		<div class="main">
			...
			<div class="main-grid3">
	           ...
	           <input type='submit' name="Submit1" style="padding: 10px; color:black; font-size: large;"  value="SEND"/>
				...
				$sql = "SELECT * FROM inbox  where un='$un' or who='$un' ORDER BY id DESC LIMIT 5  ";
                $result = $conn->query($sql);

               if ($result->num_rows > 0) {
             // output data of each row
                while($row = $result->fetch_assoc()) {
					$nid="mess.php?id=".$row["un"]."&to=".$row["who"];
                    echo '<div class="bar">';
					echo '<p>'.$row["who"].' Vs '.$row["un"].'</p>';
					echo'Sent on: '.$row["time"];
					echo '<p>Messege: '.$row["mess"].'</p>';
					echo '</div>';
				    echo '<a href="'.$nid.'" ><P><u>Click to Replay</u></p></a>';
